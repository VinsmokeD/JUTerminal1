...
async def test_tutor():
    ...
    try:
        ...
        async with AsyncSessionLocal() as db:
            ...
            stmt = sa.select(AIInteraction).where(AIInteraction.session_id == session_id).order_by(AIInteraction.created_at.desc()).limit(1)
            interaction = (await db.execute(stmt)).scalar_one_or_none()
            if interaction:
                raw_text = repr(interaction.response_text)
                sys.stdout.buffer.write(b"Raw response 1: " + raw_text.encode('ascii', errors='backslashreplace') + b"\n")
        ...
        async with AsyncSessionLocal() as db:
            ...
            if interaction:
                raw_text = repr(interaction.response_text)
                sys.stdout.buffer.write(b"Raw response 2: " + raw_text.encode('ascii', errors='backslashreplace') + b"\n")
