...
async def test_banner_does_not_trigger_domain_admin_insight():
    ...
    insights = await scan_output_chunk("test-sess", "SC-02", chunk)
...
async def test_real_domain_admin_line_triggers_insight():
    ...
    insights = await scan_output_chunk("test-sess-2", "SC-02", chunk, 4)
...
async def test_option_alone_shell_error_gets_recovery_insight():
    ...
    insights = await scan_output_chunk("test-sess-3", "SC-02", chunk)
...
async def test_missing_rockyou_gets_wordlist_recovery_insight():
    ...
    insights = await scan_output_chunk("test-sess-4", "SC-02", chunk)
...
async def test_placeholder_fallback_is_silent():
    ...
    insights = await scan_output_chunk("test-sess-5", "SC-02", "bash: syntax error near unexpected token `<'\n")
...
async def test_phase_filter_blocks_out_of_phase_sc01_schema_hint():
    ...
    phase_one = await scan_output_chunk("phase-blocked", "SC-01", "available databases [2]: novamed\n", 1)
    phase_four = await scan_output_chunk("phase-allowed", "SC-01", "available databases [2]: novamed\n", 4)
...
async def test_global_cooldown_suppresses_second_insight_within_30_seconds():
    ...
    first = await scan_output_chunk("cooldown-sess", "SC-01", "Apache/2.4.54\n", 1)
    second = await scan_output_chunk("cooldown-sess", "SC-01", "80/tcp open\n", 1)
