...
async def test_full_lifecycle_smoke():
    ...
    async with httpx.AsyncClient(timeout=30.0) as client:
        ...
        async with aconnect_ws(ws_endpoint, client) as ws:
            ...
            assert first_msg["type"] == "terminal_output", f"Expected terminal_output greeting, got: {first_msg}"
            ...
            assert siem_matched is not None, "Did not receive expected nmap SIEM event over WebSocket"
            ...
            await ws.send_json({
                "type": "tutor_question",
                "data": {"text": "How do I scan the target?"}
            })
        ...
        # Cleanup DB
        from src.db.database import AsyncSessionLocal, User, Session, AIInteraction, SiemEvent, CommandLog, UserActivity
