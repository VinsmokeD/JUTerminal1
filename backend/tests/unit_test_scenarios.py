...
async def test_ai_missing_key_returns_static_socratic_command_hint(monkeypatch):
    ...
    hint = await monitor.get_ai_hint(
        "test-session",
        {"scenario_id": "SC-01", "role": "red", "phase": 1},
        "nmap -sV 172.20.1.20",
        None,
    )

    assert hint
    assert "After using nmap" in hint
    assert "curl -I" not in hint
...
async def test_ai_rate_limit_returns_static_socratic_command_hint(monkeypatch):
    ...
    hint = await monitor.get_ai_hint(
        "test-session",
        {"scenario_id": "SC-01", "role": "red", "phase": 2},
        "gobuster dir -u http://172.20.1.20",
        None,
    )
...
def test_explicit_hint_bypasses_offline_guard(monkeypatch):
    ...
    # explicit hint request â€” must return the offline tutor fallback when no key is set
        ...
        for phase_num, phase in _phase_items(spec):
            ...
            for tool in signals.get("tools_used", []):
                ...
                if gate:
                    assert gate.get("min_phase", 1) <= phase_num, (
                        ...
                        f"{gate.get('min_phase')} â€” phase unreachable (deadlock)."
                    )
            ...
            if any_tools:
                ...
                assert reachable, (
                    ...
                    f"phase â€” phase unreachable (deadlock)."
                )
