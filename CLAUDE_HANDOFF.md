...
1. Docker Desktop is online, but the build for `parallax-kali:latest` is failing. `apt-get install` cannot locate `python3`, `wireshark-common`, and `tshark` on `kalilinux/kali-rolling:latest`. 
