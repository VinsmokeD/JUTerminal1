...
| **Hint requested** | −penalty **immediately** (live) | `ws/routes._send_hint`, `scenarios/hint_engine` |
| **Methodology gate violation** (wrong-phase tool) | −5 | `ws/routes` (PTES/engine gate) |
| **Out-of-scope target** (ROE violation) | −5 | `ws/routes` + `scope_enforcer` |
| **Fast completion** | + time bonus (0–20) | `scoring/engine.compute_time_bonus` |
...
threshold → **+10**. No bonus if the session is never completed.
...
the time bonus** — it does not re-apply hint penalties.
...
## Where it surfaces
- `GET /api/scoring/{session_id}` — `{base_score, final_score, hints_used, completed}`
- `GET /api/reports/{session_id}` — consolidated debrief includes the score block
