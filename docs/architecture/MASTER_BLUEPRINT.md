# Parallax: Master Project Blueprint & Agent Directive (v2.0)
...
Parallax is a dual-perspective cybersecurity training platform designed to bridge the gap between theoretical knowledge and real-world application.

**The Core Problem**: Students learn offensive tools (Nmap, Metasploit) and defensive tools (Splunk, EDR) in isolated silos. They lack the "overall scene" â€” the cognitive link between an attacker's keystroke and a defender's alert.
...
> âš ï¸ **Hard Limit**: The MVP is **Three High-Fidelity Scenarios only** (SC-01, SC-02, SC-03).  
...
- **Focus**: OWASP Top 10 â€” SQLi, LFI, IDOR, File Upload exploitation
...
- **Focus**: Network/Lateral Movement â€” BloodHound recon, Kerberoasting, DCSync
...
- **Pre-seeded users**: `jsmith` (low-priv), `svc_backup` (Kerberoastable, SPN: `MSSQLSvc/nexora-fs01.nexora.local:1433`), `rgreen` (AS-REP roastable, DONT_REQ_PREAUTH), `it.admin` (Domain Admin â€” member of Domain Admins)
...
- **Blue Team**: `SiemFeed.jsx` â€” scrollable feed of severity-coded events driven by WebSocket JSON frames. Background noise events render in gray; attacker events in severity colors.
...
Kali Container â—„â”€â”€ Docker API â”€â”€â”€â”€â”€ backend/src/sandbox/manager.py
    â”‚                                  â–²
    â”‚ raw PTY proxy                    â”‚ WebSocket /ws/{session_id}
    â–¼                                  â–¼
Target Containers â”€â”€ Filebeat â”€â”€â”€â”€â–º Elastic SIEM (Elasticsearch)
    (sc01, sc02)                       â”‚
                                       â–¼ backend/src/siem/engine.py (Polling)
                                       â–¼
Browser (xterm.js) â—„â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â–º React Frontend (Blue/Red Workspaces)
```
...
- Terminal output history: Redis capped list `terminal:{session_id}:history` â€” last 500 lines, replayed on re-attach
...
- **Response**: â‰¤150 tokens, always a question or conceptual nudge, never a direct exploit command
- **System prompt source**: `ai-monitor/system_prompt.md`

---

## 4. Commercial-Grade Feature Directives (v2.0 â€” Required)
...
- **What it does**: Runs as a sidecar process inside each scenario network, generating realistic benign traffic:
  ...
  - SC-02: Simulated employee AD authentication loops (4624 events every 30â€“90s with jitter)
...
- **How it works**:
  ...
  3. Each tool is mapped to a minimum required PTES phase (e.g., `sqlmap` requires phase â‰¥ 2)
  4. Session's current phase and note count are checked against the minimum
  5. If blocked: WebSocket returns `{"type": "gate_block", "message": "..."}` â€” terminal prints red warning; AI Monitor fires a redirection prompt
- **Phase unlock conditions**: Defined per scenario in YAML under `methodology_gates:` key
- **Example**: Attempting `sqlmap` in phase 1 (Recon) â†’ blocked â†’ AI asks: *"You haven't documented what you found during enumeration. What services did you identify?"*
...
- **What it renders**: Dual-axis SVG timeline
  ...
  - Vertical connector lines show causal links (attack â†’ detection latency visible)
...
- **Backend**: `backend/src/instructor/routes.py`
  - `GET /api/instructor/sessions` â€” all sessions with user info, scenario, phase, score, hint counts
  - `GET /api/instructor/sessions/{session_id}/report` â€” download student's Markdown report
...
| # | Rule | Implementation |
|---|------|---------------|
| **1** | **Docker Air-Gapping** â€” Scenario containers have ZERO outbound internet. `0.0.0.0/0` must be unreachable. | `internal: true` on every scenario network in `docker-compose.yml`. Verified with `docker network inspect` on boot. |
| **2** | **Resource Limits** â€” Hardcoded `cpus: 0.5`, `mem_limit: 512m` on all Kali and target containers. | Applied in `sandbox/manager.py` `provision_container()`. Not in docker-compose (dynamic containers must be limited at SDK level). |
| **3** | **No Live Malware** â€” Backend simulates malware *effects* via event maps only. Zero functional ransomware, shellcode, or botnet payloads in source. | Scenario YAMLs describe behaviors. Actual payload bytes are never written to any file. |
| **4** | **Persistent Terminal Sessions** â€” Refresh must re-attach cleanly. Session container persists until explicitly destroyed. | `provision_container()` checks container existence first. `terminal:{session_id}:history` Redis capped list (last 500 lines) replayed on re-attach. |
| **5** | **Continuous Logging** â€” Every sub-phase completion appended to `docs/architecture/CONTINUOUS_STATE.md` with full Who/When/Why/Where/What & How before agent turn ends. | No STATE_SAVE without a CONTINUOUS_STATE.md entry. Antigravity validates presence before marking phase âœ… Done. |
...
|-------|-------------|--------|
| 0 | Concept, architecture, documentation | âœ… Done |
| 1 | Infrastructure skeleton (docker-compose, env) | âœ… Done |
| 2 | Backend foundation (auth, WS, session) | âœ… Done |
| 3 | Scenario engine core (state machine + YAML) | âœ… Done |
| 4 | Terminal proxy end-to-end test | âœ… Done |
| 5 | SIEM event engine | âœ… Done (112 events) |
| 11 | Background noise generator (sc01â€“sc03) | âœ… Done |
| 14 | Kill Chain Timeline (Debrief) | âœ… Done |
| 15 | Instructor Dashboard | âœ… Done |
| 16 | Terminal re-attach on refresh | âœ… Done |
| 17 | Methodology gating hard locks | âœ… Done |
| 18 | Final integration + End-to-End Tests | âœ… Done locally |
| 19-22 | Unified Memory & Telemetry | âœ… Done |

**Immediate blockers / risks**:
1. `OPENROUTER_API_KEY` in `.env` may be a placeholder â€” replace with a real OpenRouter API key for Socratic hints during live demos.
...
| Background traffic | Not specified | **Required** â€” `daemon-noise.py` per scenario |
| Methodology enforcement | Soft guidance via AI | **Hard phase locks** â€” `scope_enforcer.py` blocks commands |
| Debrief | Score + notes summary | **Dual-axis Kill Chain Timeline** (SVG, attack vs detection timestamps) |
| Instructor view | Not planned | **Required** â€” `/instructor` route, role-gated |
| Container resource limits | Mentioned in `.env` | **Hardcoded** in `sandbox/manager.py` (cpus: 0.5, mem_limit: 512m) |
| Terminal persistence on refresh | Not specified | **Required** â€” Redis capped list replay on re-attach |
| DB migrations | Not set up | **Alembic required** â€” `role` column + future schema changes |
