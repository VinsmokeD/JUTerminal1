# CyberSim — MASTER FINALIZATION PLAN ("the ultimate super plan")
...
> The two LIVE plans it absorbs: `CONTINUE_HERE.md` (eng phases A–I; A–D done) and `MOTION_POLISH_PLAN.md`
> (UI phases A–F, open). Project law in `CLAUDE.md` + `CONTINUE_HERE.md` §4 still governs every change.
...
- Real Kali terminal (`cybersim-kali:latest`), 3 scenarios SC-01/02/03 fully built, isolation proven (6/6+ blocked).
...
1. **Flag discovery is silent.** Flags carry `discovery_hint`/`on_wrong_attempt_hint` in the YAML and a manual
   `FlagSubmitWidget`, but terminal output is **not scanned for flag-shaped strings** — a student who reads the
...
5. **In-flight, uncommitted work** in the tree (motion polish) — must be landed before opening new fronts.
6. Open eng phases **E (honest coverage), F (degradation/observability), G (kill-chain evidence), H (doc truth),
   I (ws/routes refactor)** and UI phases **A–F** from `MOTION_POLISH_PLAN.md`.

**Invariants (never violate — these are how things broke before):**
...
|----|-------|-----------|------|----------------|
| **WS0** | Land in-flight work + git/GitHub hygiene | — | low | Clean tree, no lost work |
...
| **WS8** | Documentation truth pass + consolidation | WS1–WS7 | low | Single source of truth |
...
**Suggested sequencing (no deadline):** WS0 → WS1 → (WS2 ∥ WS3 ∥ WS5) → WS4 → (WS6 ∥ WS7) → WS8 → WS9 → WS10.
Parallel tracks (∥) are worktree-isolatable for background sub-agents if desired.

### ⏱ FAST-TRACK (defense ≤ 1 week) — front-load what examiners see

Examiners judge the **live demo + poster + report**, not coverage % or a refactor. Crash sequence:

| Day | Do | Skip/defer |
|-----|----|-----------|
| 0.5 | **WS0** — land in-flight work, clean tree, CI green | — |
| 1–2 | **WS1** — flag discovery nudge + mission clarity (the visible wow + headline ask) | — |
| 1 (∥) | **WS5 (A–C only)** — finish boot-hero, scroll/route integrity, a11y quick wins (first impressions) | WS5 D–F deep perf |
| 1 | **WS7 (evidence only)** — run 3 kill chains, capture walkthroughs, confirm isolation; light SIEM "looks alive" check (fold in WS2-lite) | deep container hardening |
| 0.5 | **WS4 (Debrief score_breakdown only)** if time — else leave the working client score | full analytics rework |
| 2 | **WS9** — render 22 diagrams on-theme, rebuild PDF, polish + export **A2 & A3 poster PDFs**, build defense `.pptx` | — |
| 0.5 | **WS10** — full live walkthrough + screenshots; tag `v1.0.0-rc1` | — |

**Defer past the defense:** WS6 (coverage/degradation/refactor — invisible to examiners), WS3 deep tutor work
(it's already live — just confirm it responds), WS8 full doc consolidation (do a *light* truth-pass only: one
consistent score, no SC-04/05 in active docs). **Verify the AI tutor + SIEM feed both visibly work in the demo
path — a silent tutor or dead feed reads worse than any missing refactor.**

---

## WS0 — Land in-flight work + git/GitHub hygiene  *(do first)*
...
**Prompt:**
> 1. Run `npm --prefix frontend run verify` (build + 47 tests) and the backend suite per CONTINUE_HERE §6.
...
## WS1 — Flag & mission clarity  *(headline feature)*

**Why:** Owner's explicit ask — "flags should be clear when a user finds them and hinted that this is a flag."
...
> Do **not** auto-capture (the student must still submit — keeps the learning loop) but mark it discovered.
...
>    in the terminal (or a glowing inline chip) and show a non-blocking nudge: *"🚩 Looks like a flag — submit it
...
>    a clear "Phase N complete → Phase N+1 unlocked" banner.
>
...
## WS2 — SIEM realism, capture-all, live region

**Why:** "siem feed and logs to capture all and real." Verify every command→event mapping fires and reaches the
...
>    confirm each `trigger_regex` actually produces an event end-to-end (terminal → Redis pub/sub → WS → SiemFeed)
>    **and** is shipped to Elasticsearch via Filebeat (`infrastructure/docker/siem/filebeat.yml`). Add a test that
>    each rule maps to ≥1 event and that no command silently drops.
...
>    `debrief_coach.py`, `security.py` + `ai-monitor/system_prompt.md`. Confirm hints stay Socratic (Level 1→3),
>    ≤150 tokens, branch-aware, and never reveal flag values. Tighten the prompt for the 3 scenarios only.
> 2. Latency: confirm OpenRouter calls are async, cached where safe, and debounced (never per-keystroke — only on
>    command submit / note save). Add a visible "tutor thinking…" state and a graceful fallback line on timeout.
...
## WS4 — User tracking, reporting & Debrief truth
...
> 3. **Report export:** `reports/generator.py` + `learning_insights.py` → polished PDF (the frontend already has
...
> 4. **Privacy:** confirm no full terminal output is stored in Postgres (only command + metadata) — invariant.
>
...
## WS5 — UI/UX, routing, usability, performance, accessibility

**Why:** "uiux, user experience, page redirecting, ease of use, usability, performance." Finish MOTION_POLISH_PLAN
A–F and do a full UX/a11y sweep.

**Prompt:**
> Execute `MOTION_POLISH_PLAN.md` Phases A–F in order (first-paint correctness, pointer/render perf, navigation &
...
> 1. **Routing/redirects:** audit `App.jsx` RouteGuards — unauth→Auth, authed-on-public→Dashboard, deep links to a
>    workspace without a session→graceful redirect (not a blank/crash). Verify Onboarding→Dashboard→Workspace→
>    Debrief→Dashboard flows and back-button behavior. ScrollRestoration on every route.
> 2. **Usability:** consistent loading/empty/error states (Skeleton/EmptyState/ErrorBoundary already exist — apply
...
> 3. **Accessibility (WCAG 2.2 AA):** axe/Lighthouse ≥95 on Landing, Auth, Dashboard, both workspaces, Debrief;
...
> 4. **Performance:** keep the bundle split (main chunk already −56KB); lazy-load Three.js/xterm; verify perf tiers
>    downgrade on weak GPUs; Lighthouse perf ≥90 on Landing.
>
> **Gate:** all 10 routes behave; axe ≥95; Lighthouse perf ≥90 (Landing); 60fps validated under CPU throttle (not a
...
## WS6 — Backend coverage, degradation, observability, refactor

**Why:** Open eng phases E, F, I from `CONTINUE_HERE.md`.

**Prompt (run as three sub-phases, pytest green after each):**
> **E — Honest coverage:** remove/shrink the `omit` list in `backend/pyproject.toml`, record the honest baseline,
> raise `scenarios/engine|gatekeeper|branching`, `siem/engine|command_bridge` to ≥80% with focused unit tests, add
> `--cov-fail-under=<floor>` to CI.
> **F — Degradation & observability:** prove the app degrades (not crashes) when Redis/Elastic are down (add a test
...
> **I — ws/routes refactor:** FIRST add characterization tests around command pipeline / gate / hint / reconnect,
...
>    `sc02-fileserver`, `sc03-mailrelay`, `sc03-victim`) — add `no-new-privileges` + minimal `cap_add` incrementally,
...
**Why:** "documentation… remove old unnecessary unupdated docs." Sprawl is real: ~10 plan docs + 6 reviews + drift.
...
> 2. **Truth pass:** reconcile every reviewer-facing doc to the real state — one consistent score (98/100, cite
...
>    `README` reflects WS0–WS7 outcomes. Verify `CONTINUE_HERE.md` points here as the active plan.
...
## WS9 — Presentation: premium diagrams, PDF, A2/A3 poster, deck

**Why:** "very premium diagrams, pdf, A2-3 poster… top tier, matching the platform theme."

**Prompt:**
> 1. **Diagrams (22 `.mmd` in `docs/final-report/diagrams/source/`):** render ALL to high-res SVG + PNG with a
>    consistent CyberSim dark theme (Orbitron/Rajdhani headings, the poster's token palette: `--gold #C8A94A`,
...
> 2. **Graduation PDF:** rebuild `docs/final-report/formal-report/cybersim-graduation-report.pdf` from the 7
...
>    verify A2 + A3 print fidelity (it's ISO 1:1.414 so it scales), tighten the information hierarchy (problem →
>    architecture → red/blue → AI safety → results/metrics → QR to repo/demo), embed 4–6 of the rendered diagrams,
>    real metrics (334 tests, isolation, scenarios), and export print-ready **A2 and A3 PDFs**. Optionally produce a
>    Canva version via the Canva MCP for an editable handoff. Proof at 100% zoom — no overflow/clipping.
...
## WS10 — Final verification + release gate

**Prompt:**
> Full-stack verification: `docker compose up -d` → readiness all-ok; backend suite green; `npm --prefix frontend
> run verify` green; `docker compose config --quiet`; `scripts/verify-network-isolation.sh` all-blocked; a real
> browser walkthrough of all 3 scenarios (login → kill chain → flags surfaced & captured → SIEM → debrief → report).
> Lighthouse a11y ≥95 / perf ≥90 on Landing. Then tag `v1.0.0-rc1`, push, and write a release summary + a fresh
...
- [ ] All routes correct; axe ≥95; Lighthouse perf ≥90 (Landing); 60fps validated (WS5).
...
— End of plan. Start with WS0, then WS1.
