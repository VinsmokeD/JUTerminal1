...
| `backend/src/ai/monitor.py` | OpenRouter client integration and fallback handling |
...
- Apply cooldown/rate limiting per session to protect API usage.
- Fall back to static hints when OpenRouter is unavailable.
