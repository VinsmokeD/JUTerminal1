# Parallax â€” Design V5 Enhancement Plan ("Consolidate & Elevate")

> **Status:** Plan ready for execution (do later)
> **Author:** Claude (Opus 4.8) â€” 2026-05-30
...
| First draft saidâ€¦ | Reality in this repo | Consequence |
|---|---|---|
| Introduce a new palette `cyber-blue #00B4D8`, `cyber-green #00FF88`â€¦ | Palette is already `cs-red #ff3b3b`, `cs-blue #3b8bff`, `green-signal #00ff88`, plus HUD `hud-crimson #ff0055` / `hud-cyan #00f3ff` (`tailwind.config.js`, `index.css`, `v3-design.css`) | A new palette would fork a **4th** color system on top of 3 existing ones |
| Use Inter / Geist Sans | Display font is `Orbitron`â†’`Outfit`; mono is `JetBrains Mono` | Wrong typeface family entirely |
...
| Build CommandPalette, Badge, EmptyState, LiveIndicator, FlagSubmitWidget, MissionReadinessOverlay, ScoreToast, ResizableSplit, PhaseTrailâ€¦ | **All already exist** under `frontend/src/components/**` | We'd be rebuilding shipped components |
...
**The real problem is not "missing design." It is fragmentation.** There are three token sources that disagree, two color identities that fight inside single components, two button systems, dead Tailwind classes, and a shipped HUD that is heavy enough to drop frames on a projector during the defense. So V5 is **Consolidate â†’ Harden â†’ Elevate**, in that order. Net new surface area is deliberately small.
...
### 1.1 â€” Triple token drift (one value, three definitions) â€” **CRITICAL**
...
`main.jsx` imports `index.css` then `v3-design.css`, so v3 silently overrides index for the CSS-var path â€” but only *by luck of import order*, and only for vars it happens to redefine. Anyone reading `index.css` sees the wrong value. Tailwind classes (`text-txt-secondary`) and CSS-var classes (`color: var(--text-secondary)`) only agree because of this fragile patch.
**Fix:** one source of truth (Tailwind config tokens) + a single generated `:root` block; delete the duplicate/old `:root` values.

### 1.2 â€” Dual color identity, unreconciled â€” **CRITICAL**
"Duality" red/blue (`#ff3b3b` / `#3b8bff`) and "HUD neon" crimson/cyan (`#ff0055` / `#00f3ff`) coexist with no rule for which wins. They collide **inside single components**: `v3-design.css` `.btn-v3-red` uses `rgba(255,0,85,â€¦)` (HUD crimson) for border/glow but `color:#ff3b3b` (Duality red) for text; `.btn-v3-blue` mixes `#00f3ff` border withâ€¦ etc. The result reads as "two themes blended by accident," not intention.
**Fix:** adopt a documented **two-tier** rule (see Â§4). Recommendation: Duality = semantic/legible base (text, borders, severity); HUD neon = *glow/accent layer only*.

### 1.3 â€” Font split-brain â€” **HIGH**
`tailwind.config.js` `fontFamily.display = ['Orbitron','Outfit',â€¦]`; `index.css --font-display: 'Outfit'`; `DESIGN.md` says display = `Outfit`. So `.font-display` (Tailwind) renders **Orbitron**, while CSS using `var(--font-display)` renders **Outfit**. Orbitron is a geometric *display* face â€” poor for anything but short glyphs/titles; using it for body-length text hurts legibility (violates `contrast-readability` / `readable-font-size` intent).
...
### 1.4 â€” Dead / typo Tailwind classes (silently dropped) â€” **HIGH**
- `components/hints/AiHintPanel.jsx:30` â€” `'border-magenta/30 text-magenta bg-magenta/5'`: there is **no `magenta`** color in `tailwind.config.js`. The "Pro Tip" hint renders unstyled. (The inline `style` fallback at line ~368 hardcodes `#a855f7`, proving the token was meant to exist.)
- `AiHintPanel.jsx` HintBubble â€” `text-text-primary` (error & insight `bubbleBg`) is a **typo** for `text-txt-primary`; Tailwind drops it.
**Fix:** add a `magenta` token (or rename to an existing accent), fix the typo, grep for other dead classes.

### 1.5 â€” Two button systems â€” **MEDIUM**
Legacy `.btn` / `.btn-red` / `.btn-blue` / `.btn-ghost` / `.btn-sm` (solid fills, 14px) **and** `.btn-v3-*` (HUD, mono, brackets, clip-path) both live in `index.css`. `ui/Button.jsx` uses v3; legacy classes are still referenced in older pages (Landing/Auth/Debrief â€” verify with grep).
...
### 1.6 â€” Defense/projector performance risk â€” **HIGH (graduation-specific)**
`HUD_V4_AUDIT.md Â§4b` already flags it: `HudEnvironment` (three.js) + full-screen `body::before` scanline + `body::after` radial + **per-card** `card-v3::after` 8s scanline + `glitch-text` + pervasive `backdrop-filter: blur()` = heavy compositing. `PerfTier` exists but there is **no user-facing "Low Performance Mode"** (audit action item #3). A dropped-frame demo on examiner hardware is an avoidable risk.
...
### 1.7 â€” Command palette is partial â€” **MEDIUM**
`PHASE_V4_PLAN.md Â§0` + `WS-H` note the palette ships only Navigate/Scenarios/Account; Mission actions (submit flag, request hint L1/L2/L3, toggle AI mode, switch role), Tool actions (copy target IP, insert command), and Terminal actions (clear/find) are unbuilt. (vibecodecomponents.com confirms âŒ˜K grouped/searchable palette as a baseline expectation.)
**Fix:** complete the command registry; group by section.

### 1.8 â€” Accessibility gaps â€” **MEDIUM**
Good: `*:focus-visible` rings, reduced-motion kill-switch, escaped `dangerouslySetInnerHTML`, `ScenarioCard` keyboard handler. Gaps: SIEM feed has no `aria-live` (new critical events aren't announced); contrast of `txt-dim #5a6178` on `void #08090c` â‰ˆ 3.0:1 (fine for large/decorative, **fails 4.5:1** for any body text it's used on); toasts are `pointer-events:none` but not wired to `aria-live`.
**Fix:** targeted a11y sweep (Â§Phase 5).
...
- **motionsites.ai â†’** *motion vocabulary.* Staggered entrance reveals (30â€“50ms/item), layered parallax depth, glassmorphism reveals (0.6â€“0.8s), looping gradient shifts (2â€“3s), glitch/word reveals (0.3â€“0.5s), cursor-follow hero. **Tooling rule:** framer-motion for staggered containers + scroll triggers; CSS keyframes for loops; everything gated behind `will-change` + reduced-motion. â†’ Feeds **Phase 4**.
- **vibecodecomponents.com â†’** *component baseline & micro-interactions.* âŒ˜K grouped command palette, toasts auto-dismissing ~4s, directional tooltips, hover cards, backdrop-blur modals, accordions with rotating chevrons, slide-overs from edges, right-click context menus, pulsing active-state indicators, localStorage-persisted dismissible cards. Parallax already has most; this is the **completeness checklist** for Phases 3 & 6.
- **trickle.so prompt library â†’** *how to write the prompts in this doc.* Three reusable structures: **(1) Style + Component + Purpose**, **(2) Mood + Functionality**, **(3) Sensory + Technical**. Effectiveness drivers: *layered specificity* (color + type + layout + motion together), *emotion + execution*, *context clarity*. â†’ Feeds the **prompt template in Â§5**.
- **vibeui.online â†’** *layout archetypes.* Floating pill nav, terminal-as-hero, split-screen auth. Parallax already does floating/glass nav + terminal hero; reuse as reference, don't rebuild.
- **blog.vibecoder.me â†’** *workflow.* Ship in small, verifiable slices with a tight feedback loop â€” which is exactly how the phases below are scoped (each ends in a build + manual check, per `CLAUDE.md` Empirical Verification).
...
- **A11y:** body text â‰¥ 4.5:1 contrast (large/decorative â‰¥ 3:1); visible focus ring on every interactive element; icon-only buttons have `aria-label`; color never the sole signal (pair severity color with text/icon); `prefers-reduced-motion` respected.
- **Interaction:** hit targets â‰¥ 44px (or `hitSlop`/padding); press feedback within ~100ms; disabled = opacity 0.38â€“0.5 + `disabled` attr + no action; no hover-only critical actions.
...
- **Animation:** 150â€“300ms micro / â‰¤ 400ms complex; ease-out enter, ease-in exit; exit â‰ˆ 60â€“70% of enter; â‰¤ 1â€“2 animated focal elements per view; motion must convey causeâ†’effect.
- **Typography/Color:** semantic tokens, never raw hex in components; tabular figures for scores/timers/IPs; 60â€“75 char measure for prose.

---

## 4. Color + font identity â€” âœ… DECISION LOCKED: Option A (2026-05-30)

> **Confirmed by the user:** **Option A â€” "Duality base, neon accent."** Phase 0 executes against this. Do not re-open.

This is the single aesthetic call that is genuinely yours. Two coherent directions:

- **Option A â€” "Duality base, neon accent" (RECOMMENDED).** `#ff3b3b`/`#3b8bff` are the semantic/legible layer (text, borders, severity, buttons' text). `#ff0055`/`#00f3ff` become *glow/accent only* (box-shadows, laser lines, focus halos, hero). Pros: best legibility/contrast, keeps the SOC "authority" feel, smallest change. This is the professional default for a defense demo.
- **Option B â€” "Full neon HUD."** Promote `#ff0055`/`#00f3ff` to primary everywhere; retire `#ff3b3b`/`#3b8bff`. Pros: maximal cyberpunk punch. Cons: cyan-on-void can shimmer/strain, more contrast tuning, larger diff.
...
Phase 0 is written for **Option A**. If you want B, say so and I'll swap the token mapping â€” the phase structure is identical.
...
```
CONTEXT â€” read first, in order (per CLAUDE.md "Mandatory Pre-Flight Read"):
...
TASK â€” [one sentence: Style + Component + Purpose].
SCOPE â€” touch only these files: [real paths]. Do NOT introduce new color/font tokens;
  use the unified tokens from Phase 0. Reuse existing components in components/ui.
CONSTRAINTS (inherit Â§3 quality gates):
  - tokens only (no raw hex in JSX), transform/opacity animations only,
    150â€“300ms, reduced-motion safe, focus-visible, â‰¥44px targets, contrast â‰¥4.5:1 body.
ACCEPTANCE â€” [the checks from this phase] + `cd frontend && npm run build` is green.
AFTER â€” append a timestamped entry to docs/architecture/CONTINUOUS_STATE.md
...
Each phase: **Goal Â· Files Â· Findings Â· Prompt Â· Acceptance.** Phases 0â€“2 are foundation (do first, in order). Phases 3â€“6 are independent and parallel-safe.

---

### PHASE 0 â€” Token unification & identity reconciliation `[foundation Â· low effort Â· highest impact]`
**Goal:** one source of truth for color/type/elevation; encode the Â§4 two-tier rule.
...
**Prompt:**
```
Unify Parallax's design tokens into a single source of truth and encode a two-tier color identity (Option A).
...
1. In tailwind.config.js, make the token set canonical and add the missing layer:
   ...
   - Add `magenta: #a855f7` (referenced but missing â€” see AiHintPanel) OR map Pro-Tip hints to an existing accent; pick one and be consistent.
...
4. Reconcile the buttons: in v3-design.css, .btn-v3-red/.btn-v3-blue must use ONE identity per element â€”
...
### PHASE 1 â€” Defense-readiness & performance mode `[foundation Â· low Â· high (graduation)]`
...
**Prompt:**
```
Add a user-facing "Performance Mode" to Parallax and wire it through the HUD.
...
- Settings.jsx: a segmented control [Auto Â· High Â· Low] with a one-line helper:
...
```
**Acceptance:** toggling Low visibly stops the three.js background and all looping scanlines, removes blur, keeps full functionality; choice persists across reload; FPS on a throttled CPU (DevTools 6Ã—) stays smooth; `npm run build` green.

---

### PHASE 2 â€” Bug-fix & consolidation sweep `[foundation Â· low Â· medium]`
...
**Prompt:**
```
Clean up Parallax styling debt.
...
### PHASE 3 â€” Surface elevation (independent sub-tasks)
Reuse `components/ui/*`. Each sub-phase is a standalone prompt.

#### 3A â€” Terminal & HUD chrome `[medium Â· core surface]`
...
- Keep native xterm selection working (per PHASE_V4_PLAN WS-A) â€” do not regress copy/paste.
```
...
#### 3B â€” SIEM feed (Blue Team) `[medium Â· differentiator]`
**Files:** `components/siem/SiemFeed.jsx`, `__tests__/SiemFeed.test.jsx`, `.siem-event-row` CSS, `components/siem/ForensicsWorkbench.jsx`.
**Prompt:**
```
Make the SIEM feel like a live SOC console â€” and make it accessible.
- Virtualize the event list (windowing) for >50 events; reserve row height to keep CLSâ‰ˆ0.
...
- Empty state: reuse EmptyState with a sonar/radar idle (CSS, perf-gated) + "Monitoring for activityâ€¦".
...
#### 3C â€” AI Tutor panel `[medium Â· UX differentiator]`
**Files:** `components/hints/AiHintPanel.jsx`.
**Prompt:**
```
Polish the AI Tutor into a premium assistant panel (it already parses tagged hints/steps â€” keep that).
...
- After an AI message settles: a small fade-in action row (Copy / ðŸ‘ / ðŸ‘Ž) with aria-labels.
- Input: auto-grow textarea (max 4 lines); rotating context-aware placeholder ("Ask about port scanningâ€¦", "What is T1046?"); quick buttons "Hint L1 (âˆ’2)" / "Hint L2 (âˆ’5)".
- Header: LEARN/CHALLENGE mode badge + phase + branch (already present) â€” give the mode badge a sliding indicator on toggle.
...
#### 3D â€” Workspace top bar: score, flags, phase `[medium Â· most-used chrome]`
...
- Score: tabular figures; color tier 80+/60â€“79/<60 = green/amber/red; on change, count up/down + brief glow flash (green gain / red loss); animation transform/opacity only.
- Flags: "âš‘ 2/4" with â—† filled (captured) / â—‡ outline (pending); captured flash green once.
- Phase: "PHASE 2 Â· ENUMERATION" (number cs-blue, name dim) + a thin time-elapsed bar (greenâ†’amberâ†’red) â€” bar uses transform scaleX, not width.
...
- ConnectionPill: connected=green blink "LIVE", connecting=amber spinner, disconnected=red pulse "RECONNECTING attempt n/10", expired=red â†’ login. Each state has text, not just color.
```
...
#### 3E â€” Dashboard & ScenarioCard `[medium Â· first impression]`
**Files:** `pages/Dashboard.jsx`, `components/dashboard/ScenarioCard.jsx`, `components/canvas/ParticleCanvas.jsx`/`HeroScene3D.jsx`.
**Prompt:**
```
Give the dashboard a command-center hero and finish the card system (the card already has tilt+spotlight â€” keep, perf-gate it).
...
#### 3F â€” Debrief (after-action report) `[medium Â· delight + report screenshots]`
**Files:** `pages/Debrief.jsx`.
**Prompt:**
```
Turn Debrief into a mission after-action report worthy of a report screenshot.
- Score ring gauge that fills on load + number counts up from 0 (â‰¤2s, reduced-motion = instant final value).
- Score breakdown as staggered horizontal bars (base / hint penalties / gate penalties / time bonus) â€” scaleX animation, 200ms stagger.
...
#### 3G â€” Command palette completion `[medium Â· power users]`
**Files:** `components/palette/CommandPalette.jsx`, `store` (ui slice for open state).
**Prompt:**
```
Complete the âŒ˜K command registry (currently only Navigate/Scenarios/Account).
...
### PHASE 4 â€” Motion system consolidation `[medium Â· cohesion]`
...
**Prompt:**
```
Standardize Parallax motion on the existing four-curve vocabulary (enter/pop/glide tokens already in tailwind.config + v3-design.css).
...
- Audit for >500ms or linear UI transitions and bring them into 150â€“300ms.
```
...
### PHASE 5 â€” Accessibility & responsive sweep `[medium Â· correctness]`
**Goal:** meet the Â§3 gates everywhere; finish `PHASE_V4_PLAN` Phase 10.
**Files:** repo-wide sweep.
**Prompt:**
```
Run an accessibility + responsive pass against the Â§3 gates.
- Contrast: audit txt-dim (#5a6178) usages â€” if used for readable body text, bump to txt-secondary; keep dim for decorative labels only. Verify all body pairs â‰¥4.5:1 on void/surfaces.
- Keyboard: every CTA Tab-reachable in visual order; focus-visible present (already global) â€” verify on custom-styled controls (clip-path buttons, segmented toggles).
...
- Responsive: verify workspace at 1920/1440/1280/768; terminal usable â‰¥480px; no horizontal scroll; min-h-dvh over 100vh where used.
...
### PHASE 6 â€” Final micro-interaction polish `[low Â· finish]`
...
- Custom scrollbars already exist â€” verify applied to SIEM/terminal/notes/chat.
```
...
|---|---|---|---|---|
| 1 | **P0** Token unification | Everything else depends on clean tokens | â˜…â˜…â˜… | Low |
| 2 | **P1** Performance mode | Protects the live defense; cheap | â˜…â˜…â˜… | Low |
| 3 | **P2** Bug/consolidation sweep | Removes traps before building on top | â˜…â˜… | Low |
| 4 | **P3D** Top bar (score/flags/phase) | Most-used chrome | â˜…â˜…â˜… | Med |
| 5 | **P3A** Terminal | Core product surface | â˜…â˜…â˜… | Med |
| 6 | **P3B** SIEM feed | Blue-team differentiator + a11y | â˜…â˜…â˜… | Med |
| 7 | **P3C** AI Tutor | UX differentiator | â˜…â˜… | Med |
| 8 | **P3E** Dashboard/cards | First impression | â˜…â˜… | Med |
| 9 | **P3F** Debrief | Delight + report screenshots | â˜…â˜… | Med |
| 10 | **P3G** Command palette | Power feature | â˜… | Med |
| 11 | **P4** Motion consolidation | Cohesion across the above | â˜…â˜… | Med |
| 12 | **P5** A11y + responsive | Correctness gate before sign-off | â˜…â˜…â˜… | Med |
| 13 | **P6** Micro-polish | Final premium feel | â˜… | Low |

**Critical path for the defense:** P0 â†’ P1 â†’ P2 â†’ P3D â†’ P3A/P3B â†’ P5. The rest elevates but isn't load-bearing for the demo.
...
1. `cd frontend && npm run build` â†’ exit 0.
2. `cd frontend && npm test` â†’ existing suites green (Button, ConnectionPill, SiemFeed, useWebSocket); add tests where noted (SIEM aria-live).
...
**Definition of done for V5:** zero token drift (one source), one color rule documented + applied, no dead classes, a working Low-Perf mode, the Â§3 a11y gates met on Red & Blue workspaces, and all builds/tests green.
