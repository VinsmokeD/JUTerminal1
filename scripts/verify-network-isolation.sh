...
#
# Asserts Parallax's #1 safety property: scenario containers (sc01/sc02/sc03)
...
echo "== Parallax scenario network isolation check =="
mapfile -t scenario_containers < <(docker ps --format '{{.Names}}' | grep -E 'parallax[-_]sc0[1-3]' || true)
...
for c in "${scenario_containers[@]}"; do
  ...
  if [[ "$res" == "REACHABLE" ]]; then
    echo "  [BREACH] $c CAN reach ${PROBE_HOST}:${PROBE_PORT} â€” isolation FAILED"
    fail=1
  else
...
if [[ $fail -ne 0 ]]; then
  echo "RESULT: ISOLATION BREACH DETECTED â€” do not run live sessions until fixed."
  exit 1
fi
